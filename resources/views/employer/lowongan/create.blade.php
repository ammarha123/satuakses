...
@section('content')
    ...
    <form action="{{ route('employer.lowongan.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <small class="text-muted">Nama perusahaan otomatis.</small>
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="dekskripsi" class="form-control" rows="4" required>{{ old('dekskripsi') }}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="tipe_pekerjaan" class="form-control">
                @foreach(['Full-time','Part-time','Remote','Hybrid'] as $tp)
                    <option value="{{ $tp }}" @selected(old('tipe_pekerjaan')==$tp)>{{ $tp }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="fasilitas_disabilitas" class="form-control" rows="2">{{ old('fasilitas_disabilitas') }}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="batas_lamaran" class="form-control" value="{{ old('batas_lamaran') }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="is_terbuka" class="form-control">
                <option value="1" @selected(old('is_terbuka','1')==='1')>Ya</option>
                <option value="0" @selected(old('is_terbuka')==='0')>Tidak</option>
            </select>
        </div>
